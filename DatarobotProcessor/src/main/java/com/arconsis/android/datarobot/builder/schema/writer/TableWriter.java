...
public class TableWriter implements Writer {
	 ...
	 public String write() {
		 ...
		 addSqlStatment(builder, columnSql);
	 ...
	 private void addColumns(final StringBuilder builder, final List<String> columnSql, final List<String> columnNames, final List<String> toOneAssociationNames) {
		 int columnIdx = 0;
	 ...
	 private void addSqlStatment(final StringBuilder builder, final List<String> columnSql) {
		 StringBuilder statment = new StringBuilder("CREATE TABLE ");
		 statment.append(table.getName()).append(" (");
		 statment.append(concat(columnSql, new None()));
		 statment.append(")");
		 ...
		 index.append(table.getName().toLowerCase()).append(INDEX_SUFFIX);
		 index.append(" on ").append(table.getName()).append(" (");
		 index.append(concat(filterIndexableColumns(), new None()));
		 index.append(")");
