	 ...
	 private String getUsedType(final VariableElement association, final String fieldType, final boolean isCollectionType) {
		 if (!isCollectionType) {
			 if (entityNames.contains(fieldType)) {
				 ...
				 messager.printMessage(Kind.ERROR, "The @Association has to be another @Entity or Collection of another @Entity", association);
				 return "";
			 }
		 } else {
			 ...
			 if (matcher.find()) {
				 ...
				 messager.printMessage(Kind.ERROR, "No raw type is allowed when using @Association on a Collection", association);
				 return "";
			 }
		 }
	 }
